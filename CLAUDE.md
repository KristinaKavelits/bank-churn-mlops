...
5. **Registry & Shadow Deployment (`src/registry.py`)**: Handles MLflow model registry (Staging vs Production) and shadow deployment logic. Evaluates challenger vs champion on a shared holdout set; promotes challenger if F1 improves.
6. **Orchestration (`main.py`)**: Intentionally empty. End-to-end orchestration is handled by `.github/workflows/pipeline.yml` via GitHub Actions.
